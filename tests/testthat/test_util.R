...
test_that("identifiers can be extracted", {
  x <- file.path(system.file("tests", "testfiles", package = "arcticdatautils"), "example-field-projects-file.xml")
  identifier <- extract_local_identifier("field-projects", x)
  expect_equal(identifier, "215.001")

  x <- file.path(system.file("tests", "testfiles", package = "arcticdatautils"), "example-gateway-file.xml")
