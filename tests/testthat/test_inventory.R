...
test_that("an inventory can be populated with files", {
  ...
  result <- inv_load_files(inv,
                           file.path(system.file("tests", "data", "inventory", package = "arcticdatautils"), "files_simple.txt"))
  expect_true(nrow(result) == 3)

  # Case 2: Non-empty inv, non-empty file, where some are the same
  inv <- read.csv(file.path(system.file("tests", "data", "inventory", package = "arcticdatautils"), "inventory_simple.csv"), stringsAsFactors = FALSE)
  result <- inv_load_files(inv,
                           file.path(system.file("tests", "data", "inventory", package = "arcticdatautils"), "files_simple.txt"))
...
test_that("an inventory can be populated with byte sizes", {
  ...
  result <- inv_load_sizes(inv,
                           file.path(system.file("tests", "data", "inventory", package = "arcticdatautils"), "sizes_simple.txt"))
...
test_that("an inventory can be populated with checksums", {
  ...
  inv_with_checksums <- inv_load_checksums(inv,
                                           file.path(system.file("tests", "data", "inventory", package = "arcticdatautils"), "checksums_simple.txt"))
